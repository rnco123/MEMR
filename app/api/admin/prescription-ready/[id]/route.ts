import { NextResponse } from 'next/server'
import { z } from 'zod'
import { createAdminClient } from '@/lib/supabase/admin'
import {
  NotFoundError,
  ValidationError,
  handleApiError,
} from '@/lib/api-error-handler'
import { requireAdminUser } from '@/lib/admin-auth'
import { auditPhi } from '@/lib/audit-phi'

export const dynamic = 'force-dynamic'

const bodySchema = z.object({
  admin_status: z.enum(['pending', 'sent_to_pharmacy']),
})

function parseId(raw: string | undefined): number {
  const id = Number(raw)
  if (!Number.isFinite(id) || id <= 0) throw new ValidationError('Invalid id')
  return id
}

export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    const rowId = parseId(params.id)
    const user = await requireAdminUser()

    let body: unknown
    try {
      body = await request.json()
    } catch {
      throw new ValidationError('Invalid JSON body')
    }

    const parsed = bodySchema.safeParse(body)
    if (!parsed.success) throw parsed.error

    const admin = createAdminClient()
    const { data, error } = await admin
      .from('prescription_ready')
      .update({
        admin_status: parsed.data.admin_status,
        admin_status_updated_at: new Date().toISOString(),
        admin_status_updated_by: user.id,
      })
      .eq('id', rowId)
      .select('id, admin_status, admin_status_updated_at')
      .maybeSingle()

    if (error) throw error
    if (!data) throw new NotFoundError('Prescription queue row not found')

    auditPhi({
      user,
      role: 'admin',
      action: 'prescription_updated',
      resourceType: 'prescription',
      resourceId: rowId,
      metadata: { scope: 'admin_queue', admin_status: parsed.data.admin_status },
      request,
    })

    return NextResponse.json({ success: true, data })
  } catch (e) {
    return handleApiError(e)
  }
}

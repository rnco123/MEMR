...
- Normalizes and validates **addresses** via **Mapbox** (geocoding, autocomplete, suggestions, standardized components).
...
| `POST` | `/v1/addresses/resolve` | Optional sync helper: `{ freeform_or_components, country }` → Mapbox suggestion with standardized components (rate-limited). |
...
6. **Address pipeline:** Mapbox for geocode + standardization candidate; use the returned context (region code, postcode) for **US** component validation where `address_policy` requires it.
...
5. Addresses: implement `AddressPipeline` — Mapbox Geocoding/Search API; map the response context into a single recommendation object; handle non-US addresses with clear flags.

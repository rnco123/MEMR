...
- `VONLINKAGE_BASE_URL` - Base URL of the VonLinkage API, no trailing slash
- `VONLINKAGE_API_KEY` - Tenant API key (`vlk_...`). **Server-only, never
  ...
  different rooms.
- `VONLINKAGE_LIVEKIT_ORIGIN` - Origin the browser opens its LiveKit WebSocket to,
  e.g. `wss://livekit.example.com`. This is the realtime host, not the API host above.
  It is read at **build time** and baked into the Content-Security-Policy; if it is
  unset or wrong, the browser blocks the connection and no call can start.
